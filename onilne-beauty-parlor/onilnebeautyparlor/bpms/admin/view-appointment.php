...
<html>
<head>
<title>Beauty Blush || View Appointment</title>
...
<body class="cbp-spmenu-push">
    ...
    echo $msg;
						...
						<?php
$cid=$_GET['viewid'];
//$mail=mysqli_query($con,"select email from  tblappointment where ID='$cid'")
...
if($row['Status']=="1")
{
  $command = "python pyconnect.py " . escapeshellarg($row['Email']) ;
$output = exec($command);
echo $output;
  echo "Selected";
}
...
if($row['Status']=="2")
{
  $command = "python pyconnectorRe.py " . escapeshellarg($row['Email']) ;
$output = exec($command);
echo $output;
  echo "Rejected";
}
